...
def getKeyIVFromLoader(loader: bytes, version: tuple[int, int]) -> tuple[bytes, int]:
    ...
    return (
        base64.b64decode(loaded.co_consts[6]),
        base64.b64decode(loaded.co_consts[7]),
    )
...
def main(file: classes.Stub) -> dict:
    ...
    for inst in stubInsts:
        if inst.opname != "LOAD_CONST":
            continue
        if not isinstance(inst.argval, xdis.Code13) and not isinstance(
            inst.argval, CodeType
        ):
            continue
    ...
    strings = [
        inst.argval
        for inst in xdis.Bytecode(setting_obj, Opcode).get_instructions(setting_obj)
        if inst.opname == "LOAD_CONST"
    ]
    ...
    if not C2_idx:
        raise Exception(
            "Couldn't find the C2 index for BlankGrabber (Could be using telegram that is not yet supported)"
        )
