...
@deprecated("Please use get_method() instead")
def getMethod(methodName):
    ...
    return get_method(methodName)


def get_method(methodName: str):
    """
    Get a method from the methods folder by name (Return Any method if not found)
    ...
    if not module:
        if not __debug__:
            print(
                "DEBUG: Couldn't identify the stealer. proceeding using general method"
            )

        return get_method("Any")
    func = getattr(module, "main", None)

    if not func:
        raise Exception(
            "Could'nt find the method for {methodName}, forgot to import in methods/__init__.py ?"
        )
...
def decompile(Object: str | Stub) -> dict:
    ...
    if not Object.isExe:
        found = get_method("Any")(Object)
        result = found

    result = mergeAdd(result, get_method(Object.getType())(Object))

    if len(result["webhooks"]) < 1:
        result = mergeAdd(result, get_method("Any")(Object))
